...
from bsn_rest import BcfRest, BmfRest
...
def main():
    ...
    dev = BcfRest(host=node, user='admin', password='adminadmin', debug=True)

    content = dev.get('/api/v1/data/controller/core/version/appliance')['content']
    result = dict(content=content)
    dev.delete_session_cookie()
